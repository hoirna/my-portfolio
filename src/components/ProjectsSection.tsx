...
const ShowcaseSection = ({
  ...
  const defaultItems: ShowcaseItem[] = useMemo(() => [
    {
      ...
      excerpt: "A full-stack solution built with Next.js, TypeScript, and Stripe integration for seamless company experience.",
  ...
  const displayCategories = categories.length > 1 ? categories : ["All", "Full Stack", "Front-end","Back-end", "UI/UX Design",];

  useEffect(() => {
    if (activeFilter === "All") {
      setFilteredItems(displayItems);
    } else {
      setFilteredItems(displayItems.filter(item => item.category === activeFilter));
    }
              ...
              <div className="flex gap-2">
                {item.status && (
                  <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${
                    ...
                    item.status === 'In Progress' ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400' :
                    item.status === 'Beta' ? 'bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-400' :
